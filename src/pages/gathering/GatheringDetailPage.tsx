...
import {
  closeGathering,
  deleteGathering,
  getGatheringDetailById,
  joinGathering,
  userReport,
} from '/src/services/gatheringApi';
...
export function GatheringDetailPage() {
  ...
  useEffect(() => {
    ...
    if (gatheringDetail) {
      console.log('모임 생성 날짜/detail 페이지', gatheringDetail.date);
        ...
        <div className={`${styles.containerRow}  ${styles.deleteEditContainer}`}>
          {isSameUser && (
            <>
              <EditBtn
                onClick={() => {
                  if (gatheringDetail) handleEditGatheringDetail(gatheringDetail?.meetingId);
                }}
              />
              <DeleteBtn
                onClick={() => {
                  if (gatheringDetail) handleDeleteGatheringDetail(gatheringDetail?.meetingId);
                }}
              />
            </>
          )}
              ...
              {gatheringDetail?.participants.map((participant, index) => (
                <div className={styles.profileContainer} key={`${participant.userId} ${participant.userNickname}`}>
                  <div onClick={() => handleReport(index)}>
                    {isProfileClicked[index] && (
                      <div className={styles.reportBtn} onClick={() => handleClickReportBtn(participant.userId)}>
                        신고
                      </div>
                    )}
