...
import { GatheringDetailType } from '/src/types/gatheringTypes';
...
export function GatheringDetailEditPage() {
  ...
  function handleDataEdit(key: string, e: ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) {
    setGatheringData((prevData: GatheringDetailType) => ({ ...prevData, [key]: e.target.value }));
  }
  ...
  const handleDateChange = (date: Date) => {
    if (date) {
      ...
      setGatheringData((prevData: GatheringDetailType) => ({ ...prevData, date: formattingDate(date) }));
    }
  };
  ...
  function handleCreateBtn() {
    ...
    gatheringData.tags.forEach((tagItem: string, index: number) => {
        ...
        <div className={styles.containerRow}>
          ...
          <div className={styles.imageContainer}>
            ...
            <label htmlFor="image" className={styles.center}>
              {gatheringDetail && imgFileUrl && <img src={imgFileUrl} className={styles.image}></img>}
            </label>
          </div>
        </div>
        <input
