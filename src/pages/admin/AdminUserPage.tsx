    ...
    <div className={styles.adminPageContainer}>
      ...
      <div className={styles.searchContainer}>
        ...
        <SearchInput
          ...
          placeholder="회원 이름 또는 닉네임을 검색하세요"
          onChange={handleInputChange}
          value={inputValue}
        />
      </div>
      {userDataList &&
        userDataList.map((userList: UserListType, index: number) => (
          <ListComponent
            key={index}
            title={`${userList.name}(${userList.nickname})`}
            subtitle={userList.email}
            report={userList.reportCount}
            isLast={userData && userData?.totalPage >= page && userDataList.length === index + 1}
            setPage={setPage}
